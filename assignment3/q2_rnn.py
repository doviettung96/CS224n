...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) > max_length:
            ...
            aug_sentence = sentence + ([zero_vector] * diff)
            aug_labels = labels + ([zero_label] * diff)
            ret.append((aug_sentence, aug_labels, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h = tf.zeros((tf.shape(x)[0], self.config.hidden_size)) # None, hidden_size
        ### END YOUR CODE

        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)h(t)
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                output, h = cell(x[:, time_step, :], h, "RNN")
                h_drop = tf.nn.dropout(output, self.dropout_placeholder) # same as above
                pred = tf.matmul(h_drop, U) + b2 # None, n_classes
                preds.append(pred)
                ### END YOUR CODE
