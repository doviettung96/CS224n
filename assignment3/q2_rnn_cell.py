...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            self.W_x = tf.get_variable("W_x", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.W_h = tf.get_variable("W_h", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.b = tf.get_variable("b", initializer=tf.zeros(self.state_size))
            new_state = tf.nn.sigmoid(tf.matmul(state, self.W_h) + tf.matmul(inputs, self.W_x) + self.b)
            ### END YOUR CODE ###
