...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        augmented_sentence = [start] * window_size
        augmented_sentence.extend(sentence)
        augmented_sentence.extend([end] * window_size)
        for i in range(len(sentence)):
            window = []
            for word in augmented_sentence[i: i + 2 * window_size + 1]:
                window += word
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        self.W = tf.get_variable("W", shape=(self.config.n_window_features * self.config.embed_size, self.config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        ...
        h = tf.nn.relu(tf.matmul(x, self.W) + b1) # batch, H
        h_drop = tf.nn.dropout(h, self.dropout_placeholder) # same as above
        pred = tf.matmul(h, U) + b2 # batch, n_classes
        ### END YOUR CODE
        return pred
