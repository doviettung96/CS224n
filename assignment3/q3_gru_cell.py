...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            self.U_r = tf.get_variable("U_r", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.W_r = tf.get_variable("W_r", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.b_r = tf.get_variable("b_r", initializer=tf.zeros(self.state_size)) 
            r_t = tf.nn.sigmoid(tf.matmul(inputs, self.U_r) + tf.matmul(state, self.W_r) + self.b_r)
            
            self.U_z = tf.get_variable("U_z", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.W_z = tf.get_variable("W_z", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.b_z = tf.get_variable("b_z", initializer=tf.zeros(self.state_size)) 
            z_t = tf.nn.sigmoid(tf.matmul(inputs, self.U_z) + tf.matmul(state, self.W_z) + self.b_z)
            
            self.U_o = tf.get_variable("U_o", shape=(self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.W_o = tf.get_variable("W_o", shape=(self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            self.b_o = tf.get_variable("b_o", initializer=tf.zeros(self.state_size)) 
            o_t = tf.nn.tanh(tf.matmul(inputs, self.U_o) + r_t * tf.matmul(state, self.W_o) + self.b_o)
